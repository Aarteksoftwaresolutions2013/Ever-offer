function convertIdIntoBase64(offerId) {
            var encodedId = Base64.encode(offerId.toString());
            window.open("showOfferDetails?offerId="+encodedId, '_blank');
        }
(function($) {
    var that = this;
    this.easyLocatorMethods = {
            ...
            if (typeof google === 'object' && typeof google.maps === 'object') {
                ...
                if (typeof this.options.apiKey !== 'undefined') {
                    scriptMapUrl = 'https://maps.googleapis.com/maps/api/js?libraries=places&signed_in=true'
                                    + '&signed_in=true&language=en&key=' + this.options.apiKey
        ...
        loadMap : function() {
            ...
            this.options.markerClusterer = new MarkerClusterer(this.options.map, null,
                            this.options.markerClustererOptions);
        ...
        filterList : function(input) {
            ...
            if (filterTerm != '') {
                $('.js-locatorMap_list').find('.locatorMap_list_item_title:not(:contains_("' + filterTerm + '"))')
                                .parent().slideUp();
                $('.js-locatorMap_list').find('.locatorMap_list_item_title:contains_("' + filterTerm + '")').parent()
                                .slideDown();
            } else {
        ...
        getJsonData : function() {
            var script = document.createElement('script');
            script.type = 'text/javascript';
            script.src = 'https://spreadsheets.google.com/feeds/list/' + this.options.spreadsheetId
                            + '/od6/public/values?hl=en_US&alt=json'
        ...
        getContentITemList : function(i, title) {
            var htmlContent = '<li class="locatorMap_list_item" data-indexarray="' + i + '">'
                            ...
                            + ' <span class="locatorMap_list_item_title">' + title + '</span>' + '</li>';
            return htmlContent;
        },
        ...
        loadMyLocations : function() {
            ...
            for (var i = 0; i < this.options.myLocations.length; i++) {
            	...
            	var iconBase = 'http://aas-swsolution.rhcloud.com/images/newpinpoint.png';
                var marker = new google.maps.Marker({
        ...
        attachEventLocations : function() {
            ...
            $('.locatorMap_list_item').on('click', function() {
                ...
                if ($(window).width() <= 768) {// according to
                    // media query
                    $('.js-locatorMap_listContaineMobile').slideToggle("fast");
                }
        ...
        openInfoWindow : function(location) {
            ...
            locationLinkThree = location.link.split("||")[0];
            locationId = location.link.split("||")[1];

            var locationName = [];
            locationName = locationLinkThree.split("@#");

            var locationIds = [];
            locationIds = locationId.split("@#");

            for (var i = 0; locationName.length > i; i++) {
           	 locationLink[i] = '<p><a href = "#" onclick=" convertIdIntoBase64('+ locationIds[i]+')">' + locationName[i]
                + '</a></p>';
           }
            ...
            if (locationLink.length == 2) {
                linkOne = locationLink[1];
                linkTwo = '';
            }

            if (locationLink.length == 3) {
                linkOne = locationLink[1];
                linkTwo = locationLink[2];
            }
            if (locationLink.length == 4) {
                linkOne = locationLink[1];
                linkTwo = locationLink[2];
                linkThree = locationLink[3];
            }

            var contentHTMl = '<div id="locatorMap_contentInfoWindow">' + locationImage
                            + '<class="locatorMap_contentInfoWindow_title"><b>' + location.title.split("at")[1]
                            + '</b><p></br>' + location.description.split("@#")[0] + '</p><p>Opening timings:  '
                            + location.description.split("@#")[1] + '-' + location.description.split("@#")[2]
                            + '</p></br><p><b>Offers:</b></br>' + linkOne + '</br>' + linkTwo + '</br>' + linkThree
                            + '</p>';
            +'</div>';
